      ...
      <section class="landing-login">
        ...
        <template v-if="!loggingIn">
          ...
          <!-- Collapsed by default - only expands once someone actually wants the
               email/password path, so the common case (Google) stays a two-line
               card instead of a full form nobody asked to see. Once open it stays
               open regardless of which of signin/register/forgot is showing. -->
          <template v-if="!emailFormOpen">
            <button type="button" class="landing-login-toggle" @click="emailFormOpen = true">
          ...
          <p class="landing-login-guest">
            Got a room code? <router-link to="/join">Join as a guest</router-link>
          </p>
        </template>
      </section>
    </div>
